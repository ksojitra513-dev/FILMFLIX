...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($action === 'add') {
        ...
        $img = mysqli_real_escape_string($con, $_POST['image_url']);
    ...
    if ($action === 'edit') {
        ...
        $sql = "UPDATE movie_offers SET movie_name='$name', movie_image='$img', original_price=$orig, discount_price=$disc, offer_tag='$tag', expiry_date='$exp' WHERE id=$id";
...
<div class="main-content">
  ...
  <div class="page-content">
    ...
    <div class="content-card">
      <div class="toolbar">
        <div class="search-box"><i class="fas fa-search"></i><input type="text" id="offerSearch" class="form-control" placeholder="Search offers..."></div>
      </div>
        ...
        <tr>
          <td>
            <div style="display:flex;align-items:center;gap:10px;">
              ...
              <?php $real_img = (strpos($o['movie_image'], 'http') === 0) ? $o['movie_image'] : '../assets/images/' . $o['movie_image']; ?>
              <img src="<?= htmlspecialchars($real_img) ?>" class="img-thumb" onerror="this.src='../assets/images/default.jpg'">
              <?php endif; ?>
          ...
          <td>
            <div style="display:flex;gap:4px;">
              ...
              <button type="button" class="btn btn-sm btn-primary" title="Edit Offer" onclick="openEditOfferModal(<?= $o['id'] ?>, '<?= htmlspecialchars(addslashes($o['movie_name'])) ?>', '<?= $o['original_price'] ?>', '<?= $o['discount_price'] ?>', '<?= htmlspecialchars(addslashes($o['offer_tag'])) ?>', '<?= $o['expiry_date'] ?>', '<?= htmlspecialchars(addslashes($o['movie_image'] ?? '')) ?>')"><i class="fas fa-edit"></i></button>
      ...
      <div class="modal-body">
        <div class="form-grid">
          ...
          <div class="form-group full">
            ...
            <input type="text" name="image_url" class="form-control" placeholder="https://... or batman.jpg" required>
          </div>
...
<script>
filterTable('offerSearch', 'offerTable');

function openViewOfferModal(mov, img, op, dp, tag, exp) {
    document.getElementById('view_title').innerText = mov;
    const realImg = (img.indexOf('http') === 0 || img.indexOf('../') === 0) ? img : '../assets/images/' + img;
    document.getElementById('view_img').src = realImg;
    document.getElementById('view_orig').innerText = "₹" + parseFloat(op).toLocaleString();
    document.getElementById('view_disc').innerText = "₹" + parseFloat(dp).toLocaleString();
...
function openEditOfferModal(id, name, orig, disc, tag, exp, img) {
        ...
        if(form) {
            form.addEventListener('submit', function(e) {
                ...
                if (img.value.trim().length === 0) {
                    showError(img, 'Image URL or path is required.');
                }
