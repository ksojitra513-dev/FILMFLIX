...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($action === 'add') {
        ...
        $poster = mysqli_real_escape_string($con, $_POST['poster_img']);

        $sql = "INSERT INTO movies (name, `poster_img`, is_new) VALUES ('$name','$poster',$is_new)";
    ...
    if ($action === 'edit') {
        ...
        $sql = "UPDATE movies SET name='$name', is_new=$is_new, `poster_img`='$poster' WHERE id=$id";
...
<div class="main-content">
  ...
  <div class="page-content">
    ...
    <div class="content-card" style="padding:12px 16px; margin-bottom:20px;">
      <div class="search-box"><i class="fas fa-search"></i><input type="text" id="mSearch" class="form-control" placeholder="Search movies by title..."></div>
    </div>

    <!-- Movies Grid -->
    <div id="mGrid" style="display:grid;grid-template-columns:repeat(auto-fill,minmax(200px,1fr));gap:20px;">
    ...
    <div class="content-card" style="padding:0;overflow:hidden;position:relative;">
      ...
      <div style="height:200px;background:#1a1a28;overflow:hidden;">
        <?php
        $poster = $m['poster_img'];
        $src = (strpos($poster, 'http') === 0 || strpos($poster, '../') === 0) ? $poster : '../' . $poster;
        ?>
        <?php if($poster): ?>
        <img src="<?= htmlspecialchars($src) ?>" alt="<?= htmlspecialchars($m['name']) ?>"
      ...
      <div style="padding:14px;">
        <div style="display:flex; justify-content:space-between; align-items:center; margin-bottom:4px;">
           <span style="font-size:10px;color:var(--text-muted);font-weight:600;">ID: <?= $m['id'] ?></span>
           <span class="status-badge <?= strtolower($m['status'] ?? 'active') ?>" style="font-size:9px; padding:2px 6px;"><?= ucfirst($m['status'] ?? 'active') ?></span>
        </div>
        <div style="font-weight:600;font-size:14px;margin-bottom:6px;"><?= htmlspecialchars($m['name']) ?></div>
        <div style="display:flex; gap:6px; margin-top:8px;">
          <button type="button" class="btn btn-sm" style="flex:1;background:rgba(6,182,212,0.15);color:#06b6d4;border:none;" title="View Details" onclick="openViewMovieModal(<?= $m['id'] ?>, '<?= htmlspecialchars(addslashes($m['name'])) ?>', '<?= htmlspecialchars(addslashes($m['poster_img'])) ?>', <?= $m['is_new'] ?>, '<?= strtolower($m['status'] ?? 'active') ?>')"><i class="fas fa-eye"></i></button>
          <button type="button" class="btn btn-sm btn-primary" style="flex:1;" title="Edit" onclick="openEditMovieModal(<?= $m['id'] ?>, '<?= htmlspecialchars(addslashes($m['name'])) ?>', '<?= htmlspecialchars(addslashes($m['poster_img'])) ?>', <?= $m['is_new'] ?>)"><i class="fas fa-edit"></i></button>
      ...
      <div class="modal-body">
        <div class="form-grid cols-1">
          ...
          <div class="form-group">
            ...
            <input type="text" name="poster_img" class="form-control" placeholder="e.g. movie.jpg or https://..." required>
          </div>
          <div class="form-group">
    ...
    <form method="POST" id="editMovieForm" enctype="multipart/form-data" novalidate>
      ...
      <div class="modal-body">
        <div class="form-grid cols-1">
          ...
          <div class="form-group">
            <label class="form-label">Poster Image URL/Path</label>
            <input type="text" name="poster_img" id="edit_movie_poster_path" class="form-control" required>
          </div>
          <div class="form-group">
...
<div class="modal-overlay" id="viewMovieModal">
  <div class="modal" style="max-width:500px;">
    ...
    <div class="modal-body" style="text-align:center;">
       ...
       <div style="font-size:11px; color:var(--text-muted); text-transform:uppercase; letter-spacing:2px; margin-bottom:4px;">Movie ID</div>
       <div id="view_movie_id" style="font-size:18px; font-weight:800; color:var(--accent); margin-bottom:10px;"></div>
       <h3 id="view_movie_name" style="font-size:24px; font-weight:800; margin-bottom:10px; color:#fff;"></h3>
       <div style="display:flex; justify-content:center; gap:10px; margin-top:15px;">
          <span id="view_movie_status" class="status-badge"></span>
...
<script>
function openViewMovieModal(id, name, poster, isNew, status) {
    document.getElementById('view_movie_id').innerText = '#' + id;
    ...
    const statusEl = document.getElementById('view_movie_status');
...
function openEditMovieModal(id, name, poster, isNew) {
    document.getElementById('edit_movie_id').value = id;
    document.getElementById('edit_movie_name').value = name;
    document.getElementById('edit_movie_poster_path').value = poster;
    document.getElementById('edit_movie_is_new').value = isNew;
    openModal('editMovieModal');
}

function openViewMovieModal(id, name, poster, isNew, status) {
    const realPoster = (poster.indexOf('http') === 0 || poster.indexOf('../') === 0) ? poster : '../' + poster;
    document.getElementById('view_movie_id').innerText = '#' + id;
    document.getElementById('view_movie_name').innerText = name;
    document.getElementById('view_movie_poster').src = realPoster;
    
    document.getElementById('view_movie_new_badge').style.display = (parseInt(isNew) === 1) ? 'block' : 'none';
...
document.addEventListener('DOMContentLoaded', () => {
    const forms = ['addMovieForm', 'editMovieForm'];
    forms.forEach(fid => {
        ...
        if(form) {
            form.addEventListener('submit', function(e) {
                ...
                const poster = this.querySelector('[name="poster_img"]');

                if(name.value.trim().length < 2) showError(name, 'Movie name is required.');
                if(poster.value.trim().length < 3) showError(poster, 'Poster URL/path is required.');
    ...
    if (mSearch && mGrid) {
        const cards = mGrid.querySelectorAll('.content-card');
