...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (!empty($email) && !empty($password)) {
        $query = "SELECT * FROM user WHERE email = '$email' AND status = 'active' LIMIT 1";
        ...
        if ($result && mysqli_num_rows($result) > 0) {
            ...
            if ($password === $user['password'] || password_verify($password, $user['password'])) {
                if ($user['role'] === 'admin') {
                    // Admin trying to login via public portal -> send to admin
                    $_SESSION['admin_id'] = $user['id'];
                    $_SESSION['admin_name'] = $user['fullname'];
                    $_SESSION['admin_email'] = $user['email'];
                    $_SESSION['admin_avatar'] = $user['image'];
            ...
            $error = "Account not found or is inactive.";
        }
    } else {
    ...
    <style>
        ...
        * { margin: 0; padding: 0; box-sizing: border-box; }
        ...
        .form-control {
            ...
            padding: 16px 18px 16px 52px; color: var(--text-primary);
            font-family: inherit; font-size: 16px;
            transition: all 0.3s cubic-bezier(0.4, 0, 0.2, 1); outline: none;
        }

        .form-control:focus {
            background: rgba(255, 255, 255, 0.07); border-color: var(--accent-purple);
        ...
        .error-box {
            ...
            color: var(--error); padding: 14px 18px; border-radius: 14px;
        ...
        .btn-submit {
            width: 100%; background: linear-gradient(135deg, var(--accent-purple), var(--accent-cyan));
            ...
            box-shadow: 0 10px 25px rgba(129, 140, 248, 0.25); margin-top: 10px;
        }
        ...
        .login-footer { margin-top: 40px; text-align: center; font-size: 14px; color: var(--text-muted); }
        .login-footer a { color: var(--accent-cyan); text-decoration: none; font-weight: 600; transition: 0.3s; }
        .login-footer a:hover { color: var(--accent-purple); text-decoration: underline; }
...
<body>
    ...
    <div class="login-card">
        <div class="login-header">
            ...
            <p class="login-subtitle">Sign in to FilmFlix to continue</p>
        </div>

        <?php if ($error): ?>
            <div class="error-box"><i class="fas fa-shield-halved"></i><span><?= $error ?></span></div>
        <?php endif; ?>
