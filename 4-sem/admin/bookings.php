    ...
    <div class="content-card">
      ...
      <div class="table-wrapper">
        <table class="data-table" id="bookingsTable">
          <thead><tr>
            <th>#</th><th>Booking ID</th><th>User ID</th><th>Movie</th><th>Show Time</th><th>Seats</th><th>Amount</th><th>Offer ID</th><th>Offer Tag</th><th>Original Amt</th><th>Method</th><th>Payment</th><th>Status</th><th>Date</th><th>Actions</th>
          </tr></thead>
          <tbody>
          ...
          <tr>
            ...
            <td><?= $b['offer_id'] ? htmlspecialchars($b['offer_id']) : '<span style="color:var(--text-muted);">—</span>' ?></td>
            <td><?php if($b['offer_tag']): ?><span class="status-badge" style="background:rgba(245,158,11,0.15);color:#f59e0b;"><?= htmlspecialchars($b['offer_tag']) ?></span><?php else: ?><span style="color:var(--text-muted);">—</span><?php endif; ?></td>
            <td><?= $b['original_amount'] ? '₹'.number_format($b['original_amount'],2) : '<span style="color:var(--text-muted);">—</span>' ?></td>
            ...
            <td>
              <div style="display:flex;gap:4px;">
                <button type="button" class="btn btn-sm" style="background:rgba(6,182,212,0.15);color:#06b6d4;border:none;" title="View Details" onclick="openViewBookingModal('<?= htmlspecialchars(addslashes($b['booking_number'])) ?>', '<?= (int)$b['user_id'] ?>', '<?= htmlspecialchars(addslashes($b['movie_title'])) ?>', '<?= htmlspecialchars(addslashes($b['seats_booked'])) ?>', '<?= htmlspecialchars(addslashes($b['total_amount'])) ?>', '<?= (int)$b['offer_id'] ?>', '<?= htmlspecialchars(addslashes($b['offer_tag'] ?? '')) ?>', '<?= htmlspecialchars(addslashes($b['original_amount'] ?? '0')) ?>', '<?= htmlspecialchars(addslashes($b['payment_status'])) ?>', '<?= htmlspecialchars(addslashes($b['booking_status'])) ?>', '<?= htmlspecialchars(addslashes($b['payment_method'])) ?>', '<?= date('d M Y, h:i A', strtotime($b['booking_date'])) ?>')"><i class="fas fa-eye"></i></button>
    ...
    <div class="modal-body">
       <div style="background:rgba(255,255,255,0.03); border:1px dashed var(--border); border-radius:16px; padding:30px; position:relative; overflow:hidden;">
          ...
          <div style="display:grid; grid-template-columns:1fr 1fr; gap:20px; border-top:1px solid rgba(255,255,255,0.05); padding-top:20px;">
             <div>
                <label style="display:block; font-size:11px; color:var(--text-muted); text-transform:uppercase;">User ID</label>
                <div id="vb_user" style="font-size:16px; font-weight:600; color:#fff;"></div>
             </div>
             <div>
             ...
             <div>
                <label style="display:block; font-size:11px; color:var(--text-muted); text-transform:uppercase;">Original Amount</label>
                <div id="vb_original" style="font-size:16px; font-weight:600; color:#fff;"></div>
             </div>
             <div>
                <label style="display:block; font-size:11px; color:var(--text-muted); text-transform:uppercase;">Offer</label>
                <div id="vb_offer" style="font-size:14px; font-weight:600; color:#f59e0b;"></div>
             </div>
             <div>
...
function openViewBookingModal(id, userId, movie, seats, amount, offerId, offerTag, originalAmt, pay_status, book_status, method, date) {
    document.getElementById('vb_id').innerText = "#" + id;
    document.getElementById('vb_user').innerText = "#" + userId;
    ...
    document.getElementById('vb_original').innerText = parseFloat(originalAmt) > 0 ? "₹" + parseFloat(originalAmt).toLocaleString() : "—";
    document.getElementById('vb_offer').innerHTML = offerTag ? `<span style="background:rgba(245,158,11,0.15);padding:3px 10px;border-radius:6px;">#${offerId} — ${offerTag}</span>` : '<span style="color:var(--text-muted);">No offer applied</span>';
