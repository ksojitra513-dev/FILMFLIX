...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($action === 'add') {
        ...
        $sql = "INSERT INTO theaters (name, address, city, state, phone, total_screens, amenities, status) 
                VALUES ('$name','$address','$city','$state','$phone',$total_screens,'$amenities','$status')";
    ...
    if ($action === 'edit') {
        ...
        $sql = "UPDATE theaters SET name='$name', address='$address', city='$city', state='$state', phone='$phone', 
                total_screens=$total_screens, amenities='$amenities', status='$status' WHERE id=$id";
      ...
      <div class="table-wrapper">
        <table class="data-table" id="tTable">
          ...
          <tr>
            ...
            <td><?= htmlspecialchars($t['city']) ?>, <?= htmlspecialchars($t['state']) ?></td>
...
<div class="modal-overlay" id="viewTheaterModal">
  <div class="modal" style="max-width:550px;">
    ...
    <div class="modal-body">
       <div style="background:linear-gradient(135deg,rgba(168,85,247,0.1),rgba(6,182,212,0.1)); border-radius:16px; padding:30px; text-align:center; border:1px solid var(--border); margin-bottom:25px;">
          <div style="width:70px; height:70px; background:var(--accent); border-radius:18px; display:flex; align-items:center; justify-content:center; color:#fff; font-size:32px; margin:0 auto 15px; box-shadow:0 10px 20px rgba(6,182,212,0.3);">
             <i class="fas fa-building"></i>
          </div>
          <h2 id="view_name" style="font-size:24px; font-weight:800; color:#fff; margin:0;"></h2>
          <p id="view_location" style="color:var(--text-secondary); margin-top:5px;"></p>
          <div id="view_status_badge" style="margin-top:10px;"></div>
       </div>
...
function openViewTheaterModal(name, addr, city, state, phone, screens, amens, status) {
    document.getElementById('view_name').innerText = name;
    document.getElementById('view_location').innerText = `${city}, ${state}`;
...
function openEditTheaterModal(id, name, city, state, phone, screens, addr, amens, status) {
